            ...
            <div className="max-w-7xl absolute top-8 md:top-16 px-8 w-full z-10">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 items-center">
                    {logos.map((logo) => (
                        ...
                        >
                            <img src={logo.logo_path} alt={logo.name} className="dark:invert w-24 h-12 object-contain" />
                        </div>
                    ))}
                </div>
            </div>
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <h1 className="text-black dark:text-white text-7xl md:text-[150px] lg:text-[200px] font-bold select-none whitespace-nowrap z-10">
                    {titleText}
                </h1>
            </div>

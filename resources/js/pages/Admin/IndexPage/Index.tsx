...
export default function IndexPageManagement({ indexPage }: Props) {
    ...
    const handleAddLogo = (e: FormEvent) => {
        ...
        fetch("/admin/index-page/logos", {
            ...
            .then(async response => {
                ...
                // Try to parse as JSON, handling potential PHP error prefix
                let data;
                try {
                    // Check if response starts with HTML error (PHP error output)
                    if (text.startsWith('<') || text.startsWith('<br')) {
                        console.error('Server returned PHP error:', text.substring(0, 500));
                        throw new Error('Server error. Please check PHP logs for details.');
                    }
            ...
            .catch((errors: any) => {
                ...
                // Display a user-friendly toast with the error
                if (errors && typeof errors === 'object') {
                    if (errors.errors) {
                        const errorMessages = Object.entries(errors.errors)
                            .map(([field, messages]) => `${field}: ${Array.isArray(messages) ? messages.join(', ') : messages}`)
                            .join(', ');
                        toast.error('Failed to add logo: ' + errorMessages);
                    ...
                    if (errors && typeof errors === 'object') {
                        if (errors.errors) {
                            const errorMessages = Object.entries(errors.errors)
                                .map(([field, messages]) => `${field}: ${Array.isArray(messages) ? messages.join(', ') : messages}`)
                                .join(', ');
                            toast.error('Failed to update logo: ' + errorMessages);
                        } else if (errors.message) {
                            toast.error('Error: ' + errors.message);
                        } else {
    ...
    const handleDeleteLogo = (logoId: number) => {
        router.delete(`/admin/index-page/logos/${logoId}`, {
            onSuccess: () => toast.success('Logo deleted successfully'),
